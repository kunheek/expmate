...
def mp_print(*args, **kwargs):
    ...
    Usage:
        mp_print("Training started")  # Only prints on local rank 0
    """
    if get_local_rank() == 0:
        print(*args, **kwargs)


def mp_tqdm(iterable=None, disable=None, **kwargs):
    """Wrapper around tqdm that only shows progress bar on local_rank 0.

    This ensures that only one progress bar is shown per node in distributed training,
    avoiding cluttered output when running multi-GPU training.

    Args:
        iterable: Iterable to wrap (same as tqdm)
        disable: Override automatic disabling (if None, auto-detects based on local_rank)
        **kwargs: Additional arguments passed to tqdm

    Returns:
        tqdm instance or iterable (if tqdm not available or disabled)
    ...
    Example:
        ...
        >>>
        >>> # Manual control
        >>> for epoch in mp_tqdm(range(100), desc="Epochs", disable=False):
        >>>     train_epoch()

    Note:
        - Automatically disabled on all processes except local_rank 0
        - Falls back to regular iteration if tqdm is not installed
        - Can be manually controlled via disable parameter
    """
    try:
        from tqdm import tqdm
    except ImportError:
        # If tqdm not installed, return the iterable as-is
        if iterable is not None:
            return iterable
        # If called without iterable, return a dummy context manager
        from contextlib import nullcontext

        return nullcontext()

    # Auto-disable on non-local-master processes if not explicitly set
    if disable is None:
    ...
    if iterable is not None:
        ...
        # For manual updates: with mp_tqdm(total=100) as pbar: ...
