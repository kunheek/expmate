...
def get_git_info(repo_path: Optional[Path] = None) -> dict:
    ...
    Returns:
        dict: Git information including sha, branch, dirty status, etc.
              Returns default values if expmate.track_git is False.
    """
    # Return default values if git tracking is disabled
    if not expmate.track_git:
        return {
            "sha": "disabled",
            "sha_short": "disabled",
            "branch": "disabled",
...
def save_git_info(
    ...
    # Skip if git tracking is disabled
    if not expmate.track_git:
        return
